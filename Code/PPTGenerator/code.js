function processDataSet() {
	...
	var aMeasureList = aDataSet[4].split('|').map(measure => measure.trim());
	...
	for (let i = 6; i < aDataSet.length; i++) {
...
function beginProcessing() {
	...
	for (let i = 0; i < aAllMeasures.length; i++) {
		...
		slide.addText(getFriendlyText(aAllMeasures[i]), {
			x: 0.5,
			y: 0.7,
			w: 8,
			fontSize: 24
...
function getFriendlyText(sTitle) {
